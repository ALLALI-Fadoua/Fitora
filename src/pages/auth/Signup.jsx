...
import { Eye, EyeOff, Mail, Lock, User, Users, Briefcase } from 'lucide-react';
...
const Signup = () => {
    ...
    const handleSubmit = () => {
        ...
        setTimeout(() => {
            ...
            setUserData(newUser);
            navigate(newUser.role === 'COACH' ? '/coach' : '/learner', { replace: true });
        }, 600);
    };

    const roles = [
        { value: 'LEARNER', Icon: Users,     label: 'متدرب / Learner', desc: 'احجز جلسات وتعلم من الكوتشات'      },
        { value: 'COACH',   Icon: Briefcase, label: 'كوتش / Coach',    desc: 'قدم خدماتك التدريبية والكوتشينغ' },
    ];
            ...
            <header className="bg-black/80 backdrop-blur-md border-b border-amber-900/20 sticky top-0 z-50">
                <div className="container mx-auto px-4 py-4">
                    <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => navigate('/login')} className="text-gray-400 hover:text-amber-500 transition-colors">
                            لديك حساب؟ <span className="text-amber-500 font-semibold">تسجيل الدخول</span>
                        </button>
                    </div>
                </div>
            </header>
            ...
            <div className="container mx-auto px-4 py-16">
                <div className="max-w-md mx-auto">
                    <div className="text-center mb-8">
                        ...
                        <p className="text-gray-400 text-lg">انضم إلى منصة FITORA اليوم</p>
                    </div>
                            ...
                            <button key={value} onClick={() => handleRoleSelect(value)}
                                className={`p-4 rounded-2xl border-2 text-right transition-all ${
                                    formData.role === value
                                        ...
                                        : 'border-gray-700 bg-zinc-900 hover:border-gray-500'
                                }`}>
                                <Icon className={`w-8 h-8 mb-2 ${formData.role === value ? 'text-amber-500' : 'text-gray-400'}`} />
                                <p className={`font-bold text-sm ${formData.role === value ? 'text-amber-400' : 'text-white'}`}>{label}</p>
